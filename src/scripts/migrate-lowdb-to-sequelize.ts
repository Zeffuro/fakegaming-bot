...
async function migrateLowdbToSequelize() {
    ...
    for (const user of db.data.users) {
        const existing = await UserConfig.findOne({where: {discordId: user.discordId}});
        if (!existing) {
        ...
        if (user.league) {
            console.log('Migrating league for:', user.discordId, user.league);
            const leagueExists = await LeagueConfig.findOne({where: {discordId: user.discordId}});
            if (!leagueExists) {
