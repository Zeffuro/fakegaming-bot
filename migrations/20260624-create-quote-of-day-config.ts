...
async function createSqliteTable(context: MigrationSequelize): Promise<void> {
    await context.query(`
        CREATE TABLE IF NOT EXISTS "${TABLE}" (
            ...
            "runHourUtc" INTEGER NOT NULL DEFAULT 9,
...
export const up = async ({ context }: { context: MigrationSequelize }) => {
    ...
    await context.query(`
        CREATE INDEX IF NOT EXISTS "idx_quote_of_day_enabled_hour"
        ON "${TABLE}" ("enabled", "runHourUtc")
    `);
};

export const down = async ({ context }: { context: MigrationSequelize }) => {
    await context.query('DROP INDEX IF EXISTS "idx_quote_of_day_enabled_hour"');
