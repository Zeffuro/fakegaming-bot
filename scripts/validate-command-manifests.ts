...
function validateDescription(desc: string): string | null {
    if (desc.length < 1 || desc.length > 100) return `description length ${desc.length} out of range [1,100]`;
    return null;
}
...
async function main() {
    ...
    const all: CommandOut[] = perModule.flatMap(m => m.commands);
    ...
    if (strict) {
        for (const m of perModule) {
            if (!m.skipped && m.usedFallback) errors.push(`Module '${m.name}' has no commands.manifest and fallback parsing is disallowed in --strict mode.`);
        }
    }

    // Uniqueness across all modules
    const byName: Record<string, CommandOut[]> = {};
    for (const c of all) {
        const key = c.name;
