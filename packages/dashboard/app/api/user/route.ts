...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (!userData) {
            return NextResponse.json({ error: getRequestDashboardMessageFromRequest(req, "error.profileNotFound") }, { status: 404 });
        }

        return NextResponse.json(userData);
    } catch (error) {
        log.error({ err: error }, "Error fetching user data");
        return NextResponse.json({ error: getRequestDashboardMessageFromRequest(req, "error.profileUnavailable") }, { status: 500 });
    }
}
