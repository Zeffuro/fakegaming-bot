...
jest.spyOn(LeagueConfig, 'create').mockResolvedValue({});
describe('linkRiot command', () => {
    it('links Riot account and replies', async () => {
        const { command, mockManager, interaction } = await setupCommandWithInteraction({
            ...
            mocks: [
                ...
                },
                {
                    ...
                    factory: () => ({
                        resolveLeagueIdentity: jest.fn((_options) => Promise.resolve({
                            summoner: 'Zeffuro',
                            region: 'EUW',
        ...
        expect(interaction.editReply).toHaveBeenCalledWith(expect.stringContaining('Linked <@292685065920446469> to Riot ID: Zeffuro [EUW]'));
    });
});
