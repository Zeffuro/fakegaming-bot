...
describe('youtubeService', () => {
    ...
    describe('checkAndAnnounceNewVideos', () => {
        it('should announce new videos to subscribed channels', async () => {
            ...
            vi.spyOn(configManager.youtubeManager, 'getAllChannels').mockResolvedValue([ytChannel] as any);
            mockParserParseURL.mockResolvedValue({ items: mockFeed });

            await checkAndAnnounceNewVideos(client);

            expect(mockChannel.send).toHaveBeenCalled();
            expect(ytChannel.save).toHaveBeenCalled();
            expect(ytChannel.lastVideoId).toBe('new-video');
        });
